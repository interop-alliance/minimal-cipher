...
import type {
  CipherDecryptOptions,
  CipherEncryptOptions,
  EncryptResult
} from '../types.js'
...
export async function encrypt({
  ...
  additionalData,
  cek
}: CipherEncryptOptions): Promise<EncryptResult> {
...
export async function decrypt({
  ...
  additionalData,
  cek
}: CipherDecryptOptions): Promise<Uint8Array> {
  ...
  if (additionalData) {
    ...
    ;(decipher as crypto.DecipherGCM).setAAD(additionalData)
  }
...
export async function _encrypt({
  ...
  cek,
  iv
}: InternalEncryptOptions): Promise<EncryptResult> {
  ...
  if (additionalData) {
    // `plaintextLength` is optional at runtime for chacha20-poly1305
    ;(cipher as crypto.CipherGCM).setAAD(additionalData)
  }
  const encrypted = cipher.update(data)
  const final = cipher.final()
  const ciphertext =
    final.length > 0 ? Buffer.concat([encrypted, final]) : encrypted
