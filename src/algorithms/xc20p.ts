...
async function _generateSubkey({
  cek,
  nonce
}: {
  cek: Uint8Array
  nonce: Uint8Array
}): Promise<{ subkey: Uint8Array; iv: Uint8Array }> {
...
async function _hchacha20({
  ...
  const state: number[] = new Array(16)
  for (let i = 0; i < 4; ++i) {
  ...
  for (let i = 0; i < 4; ++i) {
    dvOut.setUint32(
      i * 4 + 16,
      (dvDst.getUint32(i * 4 + 48, LE) - state[i + 12]) | 0,
      LE
    )
  }
